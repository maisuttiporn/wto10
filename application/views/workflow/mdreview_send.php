...
<div class="span16">
	...
	<div class="span16">	
		<div class="well">
		<?php foreach($msgall as $row) :?>
			<p><strong> <?=$row->msg_datetime?></strong> 
			ข้อมความจาก <?=$row->userfrom_name?> 
			 ถึง  <?=$row->userto_name?> 
			  ผลการอนุมัติ  : <span class="label label-success"><?=$row->msg_approve?></span> 
			  </p>
			<p><strong>ข้อความ :</strong>	<?=$row->msg_body?> </p>			
		<?php endforeach;?>
		</div>
	</div>
	...
	<form id="techreview_form" action="../workflow/mdreview_submit" method="post">
	<strong>*ข้อมูลจะถูกส่งผ่านไปยัง  <strong style="color:red;"> ผู้บริหาร </strong> เพื่ออนุมัติใบเสนอราคา*</strong>
	<strong></strong>
